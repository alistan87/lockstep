...
def inject_frontmatter(entry: dict, path: Path, apply: bool) -> bool:
    ...
    # OKF lifecycle: mark a superseded revision so a reader can tell at a glance
    # which document is live, instead of inferring it from a filename suffix.
    if entry.get("status"):
        fields["status"] = entry["status"]
    if entry.get("superseded_by"):
        fields["superseded_by"] = entry["superseded_by"]
...
# These strings are PUBLISHED in every bundle index, so an overclaim here is a
# false statement in the documentation itself. The first draft made four, each
# caught by the review gate: it said audits are "never edited" in an index
# generated by a run that edits every audit in it; filed an accepted, partly
# executed work order under "NOT authoritative"; told readers that editing a
# guide "breaks no promise" in the bundle holding the promise made to a
# non-programmer; and described a bundle containing explicitly informative
# addenda as wholly "the contract". A description has to be true of everything
# it describes.
BUNDLE_BLURB = {
    "spec": ("The contract, and the material that qualifies it: the spec and its "
