...
| `adjudicated-review.tg.json` | up to ~3/round | `--arg subject= --arg scope= --arg ledger=`: two readonly reviewers under a frozen scope → ONE adjudicator re-grades (`personas/adjudicator.md`) → the **ledger gate** (`ledger_check`): cross-round memory in a repo file the model never writes — new/persisting/reported-resolved mechanically, `accepted-risk` only by an attributable human edit. The convergent shape from the OW-07 feedback (FLOW-AUTHORING "Convergent review"). |
