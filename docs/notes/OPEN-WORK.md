...
| 14 | **Event-driven dispatch (E)** (throughput §6, deferred at adoption) | Nodes ready before their wave finishes start earlier. | Trigger unfired. Note a defect in the trigger as written: it names `kind:"timing"` lines as the instrument, but those record tree ops only (`scope-*`, `heal-*`, `reads-hash`), so a layer-boundary gap cannot be seen through them. Fixing the instrument is the first step if this is ever wanted. |
