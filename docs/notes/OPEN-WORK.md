...
| 10 | **BUILT 2026-09-24 (first slice) — G3b per-node spawn budgets** (OW-07; DEVIATIONS 2026-09-24: `budget.max_spawns_per_node`, per node and per map item, every cause counted, persisted across the lineage; plus the map-width wallet forecast) | A runaway node cannot starve the rest of the run. | Interacts with heal rounds, map fan-out, and correctives. | **Trigger fired 2026-09-24** (downstream request, section A: a map audit starved the wallet, and `retry.max: 0` items timed out twice through the M4 auto-retry). First slice built; a whole-map total and a resume override for the per-node cap wait for a report that needs them. |
